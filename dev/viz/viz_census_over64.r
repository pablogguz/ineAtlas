...
mun_data <- get_census(
    ...
    ungroup() %>%
    mutate(
        pct_retirement_pension = case_when(
            pct_retirement_pension < 0 ~ 0,
            pct_retirement_pension >= 30 ~ 30,
            TRUE ~ pct_retirement_pension
        )
    )
    
mun_data %>%
    filter(pct_retirement_pension > 25) %>%
    nrow()
...
breaks_scale <- c(0, 10, 20, 30)
breaks_labels <- c("0", "10%", "20%", ">30%")

# Create the main map
main_map <- ggplot(mun_map) +
    geom_sf(
        aes(fill = pct_retirement_pension),
        color = NA
    ) + 
    scale_fill_gradientn(
        name = "Pensioners\n(% of 16+ population)",
        colors = rev(c("#67001F", "#B2182B", "#D6604D", "#F4A582", "#92C5DE", "#4393C3", "#2166AC", "#053061")),
        na.value = "#e4e4e4",
        breaks = breaks_scale,
    ...
    labs(
        ...
        caption =  "@pablogguz_ | The map shows the per cent of pensioners across Spanish municipalities in 2021. The share of pensioners is calculated
        as a proportion of population aged 16 and over. Missing values have been
