...
#* Project: ineAtlas
#* This script: Processes income heterogeneity files by sex
#*------------------------------------------------------------------------------
...
for(i in seq_along(files)) {
  ...
  # Check if this is a sex-only heterogeneity file
  if("Distribución de la renta por unidad de consumo" %in% names(df) && 
     "Sexo" %in% names(df) && 
     !("Tramos de edad" %in% names(df)) && 
     !("Nacionalidad" %in% names(df))) {
    cat("\nFound sex heterogeneity file:", basename(files[i]), "\n")
    data_list[[length(data_list) + 1]] <- df
  }
}

# Print summary
cat("\nFound", length(data_list), "sex heterogeneity files\n")
...
mun_data <- all_data %>%
  ...
  select(mun_code, mun_name, year, sex, indicator, value)
...
district_data <- all_data %>%
  ...
  select(mun_code, mun_name, district_code, year, sex, indicator, value)
...
section_data <- all_data %>%
  ...
  select(mun_code, mun_name, district_code, section_code, year, sex, indicator, value)
...
mun_data_wide <- mun_data %>%
  ...
  pivot_wider(
    id_cols = c(mun_code, mun_name, year, sex),
...
district_data_wide <- district_data %>%
  ...
  pivot_wider(
    id_cols = c(mun_code, mun_name, district_code, year, sex),
    names_from = indicator,
    values_from = value
  )
...
section_data_wide <- section_data %>%
  ...
  pivot_wider(
    id_cols = c(mun_code, mun_name, district_code, section_code, year, sex),
    names_from = indicator,
    values_from = value
  )

# Save the datasets
write_csv(mun_data_wide, file.path(gitdata, "distribution_sex/distribution_sex_municipality.csv"))
write_csv(district_data_wide, file.path(gitdata, "distribution_sex/distribution_sex_district.csv"))
write_csv(section_data_wide, file.path(gitdata, "distribution_sex/distribution_sex_tract.csv"))